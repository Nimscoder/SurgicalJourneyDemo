...
export function EscalationSheet({ session, open, onClose }: EscalationSheetProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-900/30 p-4" onClick={onClose}>
      <div
        className="mx-auto mt-16 w-full max-w-md rounded-2xl bg-white p-5 shadow-xl"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="mb-4 flex items-start justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">Escalation Detail</p>
