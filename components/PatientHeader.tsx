    ...
    <header className="rounded-2xl border border-white/60 bg-white/90 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <h2 className="text-xl font-semibold tracking-tight text-slate-900">{session.patientName}</h2>
          <p className="text-sm text-slate-600">{program.displayName}</p>
        </div>
        <div className="text-right">
          <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-semibold ${accent.soft}`}>
            {session.riskTier}
          </span>
          <p className="mt-2 text-sm font-semibold text-slate-800">{getDayLabel(session.currentDay)}</p>
        </div>
      </div>
      <div className="mt-4">
        ...
        <div className="h-2.5 rounded-full bg-slate-100">
          <div
            className={`h-2.5 rounded-full transition-all duration-500 ${accent.progress}`}
